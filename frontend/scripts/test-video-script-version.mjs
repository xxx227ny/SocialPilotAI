...
try {
  ...
  const panel=await fs.readFile(path.join(root,"src/components/video/VideoScriptVersionPanel.tsx"),"utf8"); const advancedPanel=await fs.readFile(path.join(root,"src/components/video/AdvancedVideoScriptVersionPanel.tsx"),"utf8"); const page=await fs.readFile(path.join(root,"src/pages/ContentStudioPage.tsx"),"utf8"); const feature=await fs.readFile(path.join(root,"src/config/features.ts"),"utf8");
  ...
  assert.ok(page.includes("videoScriptVersionsEnabled ?")); assert.ok(page.includes("qwenEnabled={qwenVideoScriptGenerationEnabled}")); assert.ok(page.includes("<AdvancedVideoScriptVersionPanel")); assert.ok(advancedPanel.includes("脚本版本管理（高级）")); assert.ok(advancedPanel.includes("item.product_id === productId")); assert.ok(feature.includes("VITE_ENABLE_VIDEO_SCRIPT_VERSIONS")); assert.ok(feature.includes("VITE_ENABLE_QWEN_VIDEO_SCRIPT_GENERATION")); assert.ok(!feature.includes("VITE_ENABLE_VIDEO_SCRIPT_VERSIONS ??")); staticChecks+=8;
