...
### V2-C4.2A Recommendation-Bound V2 Copy Generation

| Status | Date | Recommendation-bound contract | Persistence and execution boundary | Verified result | Known limitation | Next |
|---|---:|---|---|---|---|---|
| ✅ Completed; checkpoint pending | 2026-07-29 | Added Provider-free `POST /api/v1/products/{product_id}/v2-copy/preflight` and gated `POST /api/v1/products/{product_id}/v2-copy`. Backend computes and verifies a deterministic Recommendation digest, rebuilds the authoritative FeedbackContext, requires the exact atomic Strategy/CopyMatrix/VideoProject chain and current Context digest, and constrains Provider output to the exact ordered target-platform set. The Prompt uses trusted Product fields, the exact source Strategy and CopyMatrix, Recommendation constraints, and Backend-calculated overall/target metrics within structured untrusted-data boundaries | Independent Backend and Frontend V2 Copy gates default false and are required in addition to the existing Copy gate. Route gating precedes Provider resolution; Service gating repeats before reads. V2 execution has a separate single-use fee confirmation, synchronous lock, identity checks, and no automatic retry. Success creates exactly one CopyMatrix through the existing repository and leaves the source CopyMatrix unchanged; it creates no Strategy, VideoProject, RenderTask, Artifact, Campaign, or budget action. The saved row persists Product and Strategy only, so Recommendation/Context/source-Copy parentage remains response metadata rather than a durable version relation | V2 targeted tests: 33 passed; V2 plus existing Copy contract regression: 67 passed; Recommendation/Feedback/Growth regression: 47 passed; related Product/Strategy/Video regression: 95 passed; Campaign/Metrics/Dashboard/Demo regression: 13 passed; full default pytest: 342 passed, 3 real-provider skips, 1 known warning. Ruff, TypeScript, and repository-external Vite production build passed with 126 modules. Isolated browser smoke covered default-off, enabled success, rapid double-click, invalid output, safe failure, response-loss uncertainty, Backend-down recovery, Product/digest/Recommendation invalidation, and Presentation four-stage isolation with console 0/0 | Recommendation and its parent relation are not persisted. A reload cannot reconstruct the candidate as Recommendation-owned, and an uncertain response cannot safely use a latest CopyMatrix read as proof. No C4.2B VideoProject generation or C4.3A parent-child schema was added | V2-C4.2B Recommendation-bound V2 VideoProject |

Product-input fail-closed correction (2026-07-30): V2 Copy Preflight now treats Product name, category, description, and every selling-point item as untrusted runtime values. Missing, non-string, empty, or whitespace-only values return a safe HTTP 200 blocked Preflight with `product_input`; execution returns 409 before Provider generation or persistence. Correction gates passed with 58 V2 tests, 92 V2/Copy tests, 47 Recommendation/Feedback/Growth tests, 74 Product/Strategy/Video tests, 13 Campaign/Metrics/Dashboard/Demo tests, and 367 full tests (3 real-provider skips, 1 known warning), plus Ruff, TypeScript, repository-external Vite build, isolated Fake Provider browser checks, and Presentation four-stage regression with console 0/0. Two pre-existing ACL-protected pytest directories outside the repository remain a documented environment-cleanup exception; they are not Git worktree content and were not reused.

### V2-C4.2B Recommendation-Bound V2 VideoProject
